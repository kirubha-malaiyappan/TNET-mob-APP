...
app.post("/login", async (req, res) => {
  ...
  const sql = "SELECT email_address, hashed_password FROM user_credentials WHERE email_address = ?";
...
app.get("/locations", async (req, res) => {
  const sql = "SELECT DISTINCT factory_location FROM factory_data ORDER BY factory_location";
...
app.get("/shopfloors/:location", async (req, res) => {
  const location = req.params.location;
  console.log("Received request for shop floors at location:", location);
  const sql = `
  ...
  try {
    ...
    console.log("Fetched shop floors for location:", location, shopFloors);
    return res.status(200).json({ status: "success", shopFloors });
  } catch (err) {
...
app.get("/lines/:location/:shopFloor", async (req, res) => {
  try {
    ...
    const [results] = await pool.query(query, [location, shopFloor]);
...
// Overall Plant OEE Production Endpoint
app.get("/overallPlantOEE/:location/:shopFloor", async (req, res) => {
  try {
    ...
    const query = `
      SELECT md.ip_address, md.machine_name,
          mpmh.from_time, 
          mpmh.to_time, 
      ...
      JOIN machine_performance_metrics_hourly mpmh ON md.ip_address = mpmh.ip_address
      JOIN factory_data fd ON md.factory_id = fd.factory_id
      JOIN shop_floor_details sf ON md.shop_floor_id = sf.shop_floor_id
    ...
    const [results] = await pool.query(query, [location, shopFloor, fromTime, toTime]);
    console.log(results);
    return res.status(200).json({
      status: "success",
...
app.get("/machine-metrics/:ipAddress", async (req, res) => {
  try {
    ...
    console.log('Using times:', { formattedFromTime, formattedToTime });

    const query = `
       SELECT
          COUNT(*) AS TotalRecords,
          AVG(CASE WHEN productivity_percentage > 0 THEN productivity_percentage ELSE NULL END) AS AvgProductivity,
          ...
          ROUND((
            AVG(CASE WHEN utilization_percentage > 0 THEN utilization_percentage ELSE NULL END) *
            (SUM(good_parts_count) * 100.0 / NULLIF(SUM(actual_part_production), 0)) *
            AVG(CASE WHEN productivity_percentage > 0 THEN productivity_percentage ELSE NULL END)
          ) / 10000, 2) AS OEE_percentage
      FROM machine_performance_metrics_hourly AS a
      WHERE from_time >= ? AND to_time <= ? AND ip_address = ?`;

    console.log("Executing SQL Query...");
    const [results] = await pool.query(query, [formattedFromTime, formattedToTime, ipAddress]);
...
// Machine Metrics Table Endpoint
app.get("/machine-metrics-table/:ipAddress", async (req, res) => {
  try {
    ...
    const [results] = await pool.query(query, [ipAddress, formattedFromTime, formattedToTime]);
    console.log(`Found ${results.length} records`);
    return res.status(200).json({
      status: "success",
      metrics: results,
...
// Machine Metrics Moldwise Endpoint
